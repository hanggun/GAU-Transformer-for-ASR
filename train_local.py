...
alphabet_word = Alphabet('tfrecord_data/alphabet.txt')
...
def tfrecord_dataloader(files, train_mode=True, repeat=config.epochs, limit=None):
    ...
    if train_mode:
        dataset = (dataset.repeat(repeat).map(map_func=map_func, num_parallel_calls=tf.data.experimental.AUTOTUNE)
                   ...
                   .map(lambda x, y, m, n: tf.py_function(numpy_process, inp=[x, y, m, n],
                                                          Tout=[tf.float32, tf.int32, tf.float64, tf.int32]))
...
if __name__ == '__main__':
    ...
    if config.train_mode:
        if config.belu_callback:
            model.fit(gen_dataset, epochs=config.epochs, steps_per_epoch=1449//config.batch_size+1,
